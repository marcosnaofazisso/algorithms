...
export default function QuickSortViz({ algorithm }: QuickSortVizProps) {
  ...
  const [currentStep, setCurrentStep] = useState<QuickSortStep | null>(null);
  const [isRunning, setIsRunning] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [logEntries, setLogEntries] = useState<string[]>([]);
  const [speed, setSpeed] = useState<Speed>('normal');
  const [lastRunDurationMs, setLastRunDurationMs] = useState<number | null>(null);
  const generatorRef = useRef<AsyncGenerator<QuickSortStep> | null>(null);
  ...
  const handleStart = () => {
    ...
    generatorRef.current = quickSortGenerator([...data]);
    runNextStep();
  };
  ...
  const flowStep = currentStep?.flowStep || 'quick-start';
          ...
          <ResizablePanel defaultSize={70} minSize={40} className="diagram-container flex flex-col min-h-0">
            <Card className="w-full h-full flex flex-col min-h-0 rounded-none border-0 border-r border-gray-200 dark:border-gray-600">
              ...
              <CardContent className="diagram-content p-0 !pb-0 flex-1 min-h-0">
                <FlowDiagram currentStep={flowStep} variant="quick-sort" locked={diagramLocked} onLockToggle={() => setDiagramLocked((l) => !l)} />
              </CardContent>
            </Card>
          </ResizablePanel>
