...
import { bubbleSortGenerator, BubbleSortStep } from '@/lib/bubbleSort';
import type { Speed } from './LinearSearchViz';

const SPEED_DELAY_MS: Record<Speed, number> = { slow: 1600, normal: 800, fast: 200 };

interface BubbleSortVizProps { algorithm: Algorithm; }
...
export default function BubbleSortViz({ algorithm }: BubbleSortVizProps) {
  ...
  const generatorRef = useRef<AsyncGenerator<BubbleSortStep> | null>(null);
  ...
  const handleStart = () => {
    ...
    generatorRef.current = bubbleSortGenerator([...data]);
    runNextStep();
  };
  ...
  const flowStep = currentStep?.flowStep || 'bubble-start';
          ...
          <ResizablePanel defaultSize={70} minSize={40} className="diagram-container flex flex-col min-h-0">
            <Card className="w-full h-full flex flex-col min-h-0 rounded-none border-0 border-r border-gray-200 dark:border-gray-600">
              ...
              <CardContent className="diagram-content p-0 !pb-0 flex-1 min-h-0">
                <FlowDiagram currentStep={flowStep} variant="bubble-sort" locked={diagramLocked} onLockToggle={() => setDiagramLocked((l) => !l)} />
              </CardContent>
            </Card>
          </ResizablePanel>
