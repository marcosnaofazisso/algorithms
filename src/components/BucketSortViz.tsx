...
import { bucketSortGenerator, BucketSortStep } from '@/lib/bucketSort';
import type { Speed } from './LinearSearchViz';

const SPEED_DELAY_MS: Record<Speed, number> = { slow: 1600, normal: 800, fast: 200 };

interface BucketSortVizProps { algorithm: Algorithm; }
...
export default function BucketSortViz({ algorithm }: BucketSortVizProps) {
  ...
  const [currentStep, setCurrentStep] = useState<BucketSortStep | null>(null);
  const [isRunning, setIsRunning] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [logEntries, setLogEntries] = useState<string[]>([]);
  const [speed, setSpeed] = useState<Speed>('normal');
  const [lastRunDurationMs, setLastRunDurationMs] = useState<number | null>(null);
  const generatorRef = useRef<AsyncGenerator<BucketSortStep> | null>(null);
  ...
  const handleStart = () => {
    ...
    generatorRef.current = bucketSortGenerator([...data]);
    runNextStep();
  };
  const handleStop = () => {
    if (timeoutRef.current) { clearTimeout(timeoutRef.current); timeoutRef.current = null; }
  ...
  const flowStep = currentStep?.flowStep || 'bucket-start';
          ...
          <ResizablePanel defaultSize={70} minSize={40} className="diagram-container flex flex-col min-h-0">
            <Card className="w-full h-full flex flex-col min-h-0 rounded-none border-0 border-r border-gray-200 dark:border-gray-600">
              ...
              <CardContent className="diagram-content p-0 !pb-0 flex-1 min-h-0">
                <FlowDiagram currentStep={flowStep} variant="bucket-sort" locked={diagramLocked} onLockToggle={() => setDiagramLocked((l) => !l)} />
              </CardContent>
            </Card>
          </ResizablePanel>
